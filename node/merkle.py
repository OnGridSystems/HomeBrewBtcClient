...
def parse_merkleblock(data):
    ...
    # hashes
    count, offset = msg_parser.parse_var_int(data[84:])
    hashes = data[84 + offset:]
    hash_arr = []
    while count:
        tx_hash = hashes[:32]
        tx_hash = binascii.hexlify(tx_hash[::-1]).decode()
        hash_arr.append(tx_hash)
        hashes = hashes[32:]
        count -= 1
    result['hashes'] = hash_arr
    count, offset = msg_parser.parse_var_int(hashes)
...
if __name__ == '__main__':
    ...
    payload = msg_parser.get_payload(match_block)
    merkleblock = parse_merkleblock(payload)
    pmt = PartialMerkleTree(merkleblock['total_transactions'], merkleblock['flags'], merkleblock['hashes'])
    assert pmt.get_merkle_root() == merkleblock['merkle_root'], 'merkle_root mismatch'

    payload = msg_parser.get_payload(nomatch_block)
    merkleblock = parse_merkleblock(payload)
    pmt = PartialMerkleTree(merkleblock['total_transactions'], merkleblock['flags'], merkleblock['hashes'])
    assert pmt.get_merkle_root() == merkleblock['merkle_root'], 'merkle_root mismatch'
